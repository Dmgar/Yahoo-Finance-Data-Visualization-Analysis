...
def plot_full_analysis(
    ...
    ax = axes[0]
    ax.plot(
        df.index, df["Close"], color=ACCENT, linewidth=1.5, label="Precio", zorder=3
    )
    ax.plot(
        df.index,
        df["bb_upper"],
    ...
    ax.plot(
        ...
        linewidth=0.8,
        linestyle="--",
        label=f"BB Media ({report.window}d)",
        alpha=0.6,
    )
    ax.plot(
        df.index,
...
def plot_comparison(
    ...
    fig, axes = plt.subplots(
        1, 3, figsize=(16, max(5, len(df) * 0.7 + 2)), facecolor=DARK_BG
    )
    fig.suptitle(
        "Comparación de Volatilidad entre Pares de Divisas",
        color=TEXT_COLOR,
        fontsize=13,
        fontweight="bold",
    )
...
def _add_watermark(ax):
    ax.text(
        0.99,
        0.02,
        "forex-volatility | Yahoo Finance",
        transform=ax.transAxes,
        color=GRID_COLOR,
        fontsize=7,
        ha="right",
